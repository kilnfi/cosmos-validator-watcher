...
type VotesV1Watcher struct {
	metrics    *metrics.Metrics
	validators []TrackedValidator
	pool       *rpc.Pool
}

func NewVotesV1Watcher(validators []TrackedValidator, metrics *metrics.Metrics, pool *rpc.Pool) *VotesV1Watcher {
	return &VotesV1Watcher{
		metrics:    metrics,
...
func (w *VotesV1Watcher) Start(ctx context.Context) error {
...
func (w *VotesV1Watcher) fetchProposals(ctx context.Context, node *rpc.Node) error {
	...
	for _, proposal := range proposalsResp.GetProposals() {
		...
		for _, validator := range w.validators {
			...
			if isInvalidArgumentError(err) {
				...
				w.handleVote(chainID, validator, proposal.Id, vote.Options)
			}
		}
	}
...
func (w *VotesV1Watcher) handleVote(chainID string, validator TrackedValidator, proposalId uint64, votes []*gov.WeightedVoteOption) {
	voted := false
